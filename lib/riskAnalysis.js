...
import {
  calculateChurnSignal,
  calculateCoverageGapSignal,
  calculateIncidentHotspotSignal,
  calculateFlakeProximitySignal,
  calculateDiffRiskSignal,
  calculateTimePressureSignal
} from './signals.js';
...
export async function analyzeRisk(params) {
  ...
  const signals = {
    churn: calculateChurnSignal(files, commitHistory),
    coverage_gap: calculateCoverageGapSignal(files, coverageData),
    incident_hotspot: calculateIncidentHotspotSignal(files, incidentHistory),
    flake_proximity: calculateFlakeProximitySignal(files, flakeData),
    diff_risk: calculateDiffRiskSignal(files, pullRequest),
    time_pressure: calculateTimePressureSignal(pullRequest)
  };
